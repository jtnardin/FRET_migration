function run_sim(l,m,q_est,plot_data,save_im)
...
%     close
    ...
    title(['FRET interpolation for cell density = ' num2str(cell_dens(m-1)) ' cells/mm^2'])
    xlabel('Time (t)')
    ylabel('FRET')
    
    if save_im == 1
        exportfig(gcf,['FRET_fitting_' num2str(m) '.eps'],'color','rgb')
        saveas(gcf,['FRET_fitting_' num2str(m) '.fig'])
    end
